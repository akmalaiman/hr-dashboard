...
export class StaffEditComponent implements OnInit {
    ...
    fetchStaffById(id: number): void {
        this.staffService.getStaff(id).subscribe({
            next: (data: Staff) => {
                ...
                if (data.departmentId.id > 0) {
                    this.loadManagerList(data.departmentId.id);
                }

                if (data.reportingTo > 0) {
                    this.editStaffForm.patchValue({
                        reportingToId: data.reportingTo
                    });
                }
            },
            error: (error: any) => {
    ...
    loadManagerList(departmentId: number) {
        this.editStaffForm.get('reportingToId')?.setValue('');
    ...
    onSubmit(): void {
        ...
        if (this.editStaffForm?.valid) {
            ...
            const staffData: Staff = {
                ...
                reportingTo: this.editStaffForm.value.reportingToId ? Number(this.editStaffForm.value.reportingToId) : 0
            };
